...
module.exports = function(config) {
  ...
  return async function(content,cf) {
    ...
    //
    let result = adaptor.innerHTML(adaptor.body(html.document));
    
    if(config.fuck_math){
      const cheerio = require('cheerio');
      const $ = cheerio.load(result);
      const targetSelector1 = 'mjx-container.MathJax[display]';
      $(targetSelector1).wrap('<div class="mathformula-display"></div>');
      const targetSelector2 = 'mjx-container.MathJax:not([display])';
      $(targetSelector2).wrap('<div class="mathformula-inline"></div>');
