...
function processRule3(content) {
    ...
    while (i < n) {
        if (content[i] === '_') {
            ...
            if (i > 0 && content[i - 1] === '\\') {
                result.push('%');  // 已转义，保持原状
            } else if (i > 0 && content[i - 1] === '{') { // Nunjucks 和 Mathjax 的语法冲突
                result.push(' % ');  // 加空格
            } else {
                result.push('\\', '%');  // 未转义，添加
            }
            ...
            if (i > 0 && content[i - 1] === '}') {
                ...
                result.push(' } ');  // 加空格
            } else if (i > 0 && content[i - 1] === '%') { // Nunjucks语法冲突
                result.push(' } ');  // 加空格
            } else {
                result.push('}');  // 保持原状
            }
...
function processMathFormulas(str) {
    return str.replace(/(```.*?```)|(\$\$?)(.*?)\2/sg, (match, codeBlock, delimiter, content) => 
      codeBlock ? codeBlock : `${delimiter}${processFormulaContent(content)}${delimiter}`
    );
    /*
